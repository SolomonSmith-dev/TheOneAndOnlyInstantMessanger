...
function checkServerSecurity() {
    ...
    const serverFiles = ['server-secure.js', 'server.js'];
    let serverContent = '';
    let secureServerFound = false;
    
    for (const file of serverFiles) {
        if (fs.existsSync(file)) {
            serverContent = fs.readFileSync(file, 'utf8');
            if (file === 'server-secure.js') {
                secureServerFound = true;
                console.log('✅ Secure server file found');
            }
            break;
        }
    }
    
    if (!secureServerFound) {
        warnings.push('⚠️  Secure server file not found, checking regular server');
    }
    ...
    if (serverContent.includes('lockAccount') || serverContent.includes('failed_login_attempts')) {
        console.log('✅ Account lockout mechanism found');
        securityScore += 5;
    } else {
    ...
    if (serverContent.includes('validateSession') || serverContent.includes('session_token')) {
        console.log('✅ Session management found');
        securityScore += 5;
    } else {
    ...
    if (serverContent.includes('securityLog') || serverContent.includes('Security Event')) {
        console.log('✅ Security logging found');
        securityScore += 5;
    } else {
        warnings.push('⚠️  Security logging not found');
    }
